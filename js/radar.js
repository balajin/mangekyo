window.app.controller('radarController', ['$scope', '$http', function($scope, $http){
	...
	$http.get('http://localhost:5984/balajin/_design/radar/_view/fetch', {
		...
		for(var typeKey in $scope.response){
			radar_data = $scope.response[typeKey];
                        var y_axis = [];
                        var x_axis = [];
                        ...
                        var margin = {top: 20, right: 30, bottom: 30, left: 40},
                            width = (radar_data.length * 190),
                            height = 500 - margin.top - margin.bottom;
                        var x = d3.scale.ordinal().domain(x_axis).rangeRoundBands([0, width], .1);
                        var y = d3.scale.linear().range([height, 0]);
                        var chart = d3.select("#" + typeKey)
                            .attr("width", width + margin.left + margin.right)
                            .attr("height", height + margin.top + margin.bottom)
                            .append("g")
                            .attr("transform", "translate(" + margin.left + "," + margin.top + ")");
                        x.domain(_.map(radar_data, function(d) { return d["label"] ; }));
                        ...
                        bar.append("text").attr("x", x.rangeBand() - 40  ).attr("y",function(d) { return y( d["support"] ) - 5; } ).attr("dy", ".175em").text( function(d){ return d["opinion"];}); 
